...
# Script to generate JSON build information for ROM builds
...
if [[ -f "$file_path" ]]; then
    ...
    # Check if sha256sum file exists
    if [[ ! -f "$file_path.sha256sum" ]]; then
        echo -e "${YELLOW}Warning: SHA256 sum file not found, generating it now${NC}"
        sha256sum "$file_path" > "$file_path.sha256sum"
    fi
